...
★ **`/admin` הוא קובץ אמיתי**, לא rewrite. `public/admin.html`
מועתק לשורש ה-build ומפנה מיד ל-`/#admin`.

הגרסה הראשונה הסתמכה על `rewrites` ב-`vercel.json` — **וקיבלה
404 בייצור**. זה יכול לקרות מכמה סיבות שקשה להבחין ביניהן
מבחוץ: Root Directory שמוגדר אחרת בפרויקט, framework preset
שדורס, או פריסה שלא קלטה את הקובץ. קובץ סטטי לא תלוי באף אחת
מהן — הוא נענה מהדיסק בכל שרת שקיים.

⚠ אם משנים את `#admin` באפליקציה — לשנות גם ב-`public/admin.html`
וב-`public/admin/index.html`.
...
| **404 על `/admin` בייצור** | ה-rewrite לא נכנס לתוקף | תוקן — `public/admin.html` הוא קובץ אמיתי. לפרוס מחדש |
| **`42P16: cannot drop columns from view`** | הרצת מיגרציה מוקדמת אחרי מאוחרת | תוקן — כל התצוגות עושות `DROP` לפני `CREATE`. להריץ שוב |
