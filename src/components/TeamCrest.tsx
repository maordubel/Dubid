 ...
 * ★ למה זה לא `slice(0, 2)`
 *
 * בפורמט הקודם (`ה.תא`, `מ.נתניה`) חיתוך נאיבי החזיר `ה.` ו-`מ.`
 * — כלומר **כל קבוצות הפועל נראו זהות**, וכך גם כל מכבי. זה
 * בדיוק ההפך ממה שסמל אמור לעשות.
 *
 * בפורמט הנוכחי יש שתי צורות, ולכל אחת כלל משלה:
 *
 *   `מ·ת״א`  → אות המועדון + האות הראשונה של העיר  → `מת`
 *   `נתניה`  → שתי האותיות הראשונות                → `נת`
 *
 * גרשיים וגרש נופלים תמיד — הם סימני קיצור ולא אותיות, ובריבוע
 * של 20 פיקסלים הם רק רעש.
 */
