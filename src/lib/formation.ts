 ...
 */
/**
 * ★ המגרש התארך, כי הכרטיס התארך.
 *
 * מ-1.38 ל-1.5: הכרטיס עבר מסמל ריבועי לדמות מצוירת עם שורת
 * כפתורי סימון גלויה, כלומר גבוה בהרבה ביחס לרוחבו. בלי מגרש
 * ארוך יותר, `verticalCap` היה מכווץ כל כרטיס עד לרצפה של 44
 * פיקסלים — ואז השורות היו נדרסות זו על זו במסך צר.
 *
 * זו גם פרופורציה נכונה יותר: מגרש כדורגל אמיתי ארוך מ-1.38.
 */
...
export function ratioForFormation(formation: string): number {
  ...
  return rows.length >= 4 ? PITCH_RATIO : 1.35;
}
 ...
 *
 * ★★ המספר הזה השתנה, וזו הסיבה ★★
 *
 * הכרטיס היה: סמל ריבועי (62% מהרוחב) + שורת שם + שורת מחיר.
 * יחס 1.3.
 *
 * עכשיו הוא: **דמות מצוירת** (86% מהרוחב, ביחס 64:70) + שורת
 * שם + שורת כפתורי סימון (קפטן · סגן · הסרה) שתמיד גלויה.
 * כלומר הכרטיס גבוה בהרבה ביחס לרוחבו.
 *
 * אם המספר הזה לא היה עולה איתו, `verticalCap` היה ממשיך
 * לאשר כרטיסים רחבים מדי — והשורות היו נדרסות זו על זו. זה
 * בדיוק הבאג שנתפס פעם אחת כשהמגרש קיבל יחס נמוך יותר, ולכן
 * הנוסחה נבדקת (`tests/formation.test.ts`).
 *
 *   0.86 × (70/64) ≈ 0.94  ·  שם ~15px  ·  כפתורים ~18px
 *   ומרווחים ~8px  →  בערך 0.94w + 41px, כלומר ~1.63 ב-w=64.
 *
 * 1.7 הוא זה עם שוליים.
 */
