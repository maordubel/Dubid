...
/**
 * ★★ שעון שרץ אחורה כל השבוע הוא שעון שאיש לא מסתכל עליו. ★★
 *
 * זו לא החלטה עיצובית — זו החלטה על תשומת לב. כשנשארו שלושה
 * ימים, `72:14:07` הוא מספר גדול שלא אומר כלום: אף אחד לא
 * מתרגם אותו לימים בראש, והשניות שרצות הן רעש. ואם השעון רץ
 * תמיד, הוא מפסיק להיות סימן לדחיפות — ואז ביום האחרון,
 * כשהוא באמת דחוף, הוא נראה בדיוק כמו תמיד.
 *
 * לכן שלושה מצבים, ולא אחד:
 *
 *   `far`    — יותר מיממה. "בעוד יומיים". שקט, בלי שניות.
 *   `today`  — היממה האחרונה. השעון מתחיל לרוץ. HH:MM:SS.
 *   `urgent` — השעה האחרונה. אותו שעון, באדום.
 *
 * ★ מה שהמסך **לא** צריך לדעת: את הסף. הוא שואל את הפונקציה
 *   מה להציג, והיא עונה. סף שכתוב בשני מסכים הוא סף שיום אחד
 *   יהיה שונה בכל אחד מהם.
 */
export type DeadlineMode = 'far' | 'today' | 'urgent' | 'closed';

export interface DeadlineView {
  mode: DeadlineMode;
  /** הטקסט הראשי: "בעוד יומיים" או "07:42:15". */
  value: string;
  /** האם `value` הוא שעון רץ — כלומר לעטוף ב-`dir="ltr"`. */
  ticking: boolean;
  /** תווית מעל: "מבזק" / "היום האחרון" / "שעה אחרונה". */
  flash: string;
  /** משפט מתחת. */
  note: string;
}

/** מתי השעון מתחיל לרוץ. יממה. */
export const TICK_FROM_MS = 24 * 3600_000;
/** מתי הוא הופך לאדום. שעה. */
export const URGENT_FROM_MS = 3600_000;

export function deadlineView(ms: number): DeadlineView {
  if (ms <= 0) {
    return {
      mode: 'closed', value: 'ההרכבים נעולים', ticking: false,
      flash: 'נעול', note: 'הכדור כבר מתגלגל.',
    };
  }
  ...
  if (ms >= TICK_FROM_MS) {
    /* ★ `daysLabel` מחזיר "עוד יומיים"; כאן זה "בעוד יומיים".
       ההבדל הוא מילה אחת, והיא מה שהופך תווית לכותרת. */
    return {
      mode: 'far',
      value: daysLabel(c.days).replace(/^עוד /, 'בעוד '),
      ticking: false,
      flash: 'מבזק',
      note: 'אחרי זה אין שינויים.',
    };
  }

  if (ms >= URGENT_FROM_MS) {
    return {
      mode: 'today', value: c.clock, ticking: true,
      flash: 'היום האחרון', note: 'השעון רץ.',
    };
  }

  return {
    mode: 'urgent', value: c.clock, ticking: true,
    flash: 'שעה אחרונה', note: 'אחרי זה אי אפשר לשנות כלום.',
  };
}
